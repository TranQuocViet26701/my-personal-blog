...
# CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get("SQLALCHEMY_DATABASE_URI", "sqlite:///blog.db")
...
class Comment(db.Model):
    ...
    # Create Foreign Key, "blog_post.id" the blog_post refers to the tablename of BlogPost.
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash("The email already existed, please try again.")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You should login to comment the post.")
            return redirect(url_for('login', next=request.url))
        new_comment = Comment(body=form.comment.data, author_id=current_user.id, post_id=requested_post.id)
